...
void main() async {
  ...
  try {
    ...
    // ✅ Pre-warm the token immediately after activation
    // This prevents multiple API calls from all trying to fetch simultaneously
...
class _AppLifecycleManagerState extends State<AppLifecycleManager>
  ...
  void _routeFromDeepLink(Uri uri) async {
    ...
    if (path.contains('/register')) {
      ...
      final fromInvite = params['fromInvite'] == 'true';
    ...
    // /reset-password or /auth-action
    if (path.contains('/reset-password') || path.contains('/auth-action')) {
      final token = params['token'] ?? '';
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(
          builder: (_) => Forgotpassword(), // ← your reset screen
        ),
        (route) => false,
      );
      return;
    ...
    // Default — user logged in → home, user not logged in → login
    if (user != null) {
