...
class LocationListProvider extends ChangeNotifier {
  ...
  Future<void> deleteLocations(
    ...
    try {
      ...
      var headers = await CommonHeaders.createHeaders();
      var body = json.encode({
        "data": {"location_list": locationList}
      });

      var url = Uri.parse(
          AppConstant.ADD_ACCOUNT + "?bulk_delete_location_list=true");
      log(url.toString());
      ...
      if (response.statusCode == 200) {
        ...
        locationList.forEach((element) {
          if (_locationList.any(
              (location) => location.locationId == element["location_id"])) {
            // only decrement if present in the location list
            if (_locationList.any(
                (location) => location.locationId == element["location_id"])) {
              locationHits--;
            }
            _locationList.removeWhere(
                (location) => location.locationId == element["location_id"]);
            if (_certifiedLocationList.any(
                (location) => location.locationId == element["location_id"])) {
              certifiedLocationHits--;
            }

            _certifiedLocationList.removeWhere(
                (location) => location.locationId == element["location_id"]);
          }
        });
        notifyListeners();
